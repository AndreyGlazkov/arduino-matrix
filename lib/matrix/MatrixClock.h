...
void MatrixClock::draw(LedMatrix *matrix) {
    ...
    int hm = _minuts/10;
    int lm = _minuts%10;
    _digitPrint.draw(4*0, posY, hh, matrix, CRGB::Black);
    _digitPrint.draw(4*1, posY, lh, matrix, CRGB::Black);
    _digitPrint.draw(4*2, posY, hm, matrix, CRGB::Black);
    _digitPrint.draw(4*3, posY, lm, matrix, CRGB::Black);
};